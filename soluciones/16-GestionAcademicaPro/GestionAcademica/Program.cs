...
AnsiConsole.Write(new Rule("[lightsteelblue]━━━━━ SISTEMA DE GESTIÓN ACADÉMICA - DAW ━━━━━[/]").RuleStyle(Color.LightSkyBlue1).Centered());

// --------------------------------------------------------------------
// PANEL DE ESTADÍSTICAS DEL SISTEMA (MEJORA 5)
// --------------------------------------------------------------------
// Obtenemos estadísticas del sistema para mostrar en el inicio
var serviceProviderTemp = DependenciesProvider.BuildServiceProvider();
using var scopeTemp = serviceProviderTemp.CreateScope();
var serviceTemp = scopeTemp.ServiceProvider.GetRequiredService<IAcademiaService>();
var todos = serviceTemp.GetAll().ToList();
...
void ListarTodas(IAcademiaService service)
{
    ...
    var criterio = AnsiConsole.Ask<string>("⚙️  Criterios: [yellow]1.ID[/], [yellow]2.DNI[/], [yellow]3.Apellidos[/], [yellow]4.Nombre[/], [yellow]5.Ciclo[/]\n🎯 Seleccione criterio: ");
...
void BuscarPorDniGeneral(IAcademiaService service)
{
    ...
    AnsiConsole.Status().Start("🔎 Buscando persona...", ctx =>
    {
        service.GetByDni(dni).Match(
            onSuccess: p => ImprimirPanelPersona(p),
...
void ListarEstudiantes(IAcademiaService service)
{
    ...
    var criterio = AnsiConsole.Ask<string>("⚙️  Criterios: [yellow]1.ID[/], [yellow]2.DNI[/], [yellow]3.Apellidos[/], [yellow]4.Nombre[/], [yellow]5.Nota[/], [yellow]6.Curso[/], [yellow]7.Ciclo[/]\n🎯 Seleccione criterio: ");
    ...
    if (AnsiConsole.Confirm("¿Confirmar alta?"))
    {
        AnsiConsole.Status().Start("💾 Guardando estudiante...", ctx =>
        {
            service.Save(temp).Match(
                ...
                onFailure: error => MostrarError($"❌ ERROR: {error.Message}")
            );
        });
    }
}
...
void MostrarInformeEstudiantes(IAcademiaService service)
{
    ...
    var alcance = AnsiConsole.Ask<string>("⚙️  Alcance: [yellow]1.Global[/], [yellow]2.Por Ciclo[/], [yellow]3.Por Curso[/], [yellow]4.Clase Específica[/]\n🎯 Seleccione alcance: ");
...
void ListarDocentes(IAcademiaService service)
{
    ...
    var criterio = AnsiConsole.Ask<string>("⚙️  Criterios: [yellow]1.ID[/], [yellow]2.DNI[/], [yellow]3.Apellidos[/], [yellow]4.Nombre[/], [yellow]5.Experiencia[/], [yellow]6.Módulo[/], [yellow]7.Ciclo[/]\n🎯 Seleccione criterio: ");
...
void AnadirNuevoDocente(IAcademiaService service)
{
    ...
    { Dni = dni, Nombre = nombre, Apellidos = apellidos, Experiencia = experiencia, Especialidad = modulo, Ciclo = ciclo };
...
void ActualizarDocente(IAcademiaService service)
{
    ...
    var exp = AnsiConsole.Confirm("¿Cambiar experiencia?") ? AnsiConsole.Ask<int>("⏳ Nuevos años de experiencia: ") : doc.Experiencia;
...
void MostrarInformeDocentes(IAcademiaService service)
{
    ...
    var alcance = AnsiConsole.Ask<string>("⚙️  Alcance: [yellow]1.Global[/], [yellow]2.Por Ciclo[/]\n🎯 Seleccione alcance: ");
...
{
    AnsiConsole.Write(new Rule("[lightseagreen]🖨️ GENERANDO INFORME HTML DE ESTUDIANTES[/]").RuleStyle(Color.LightSeaGreen));
...
{
    AnsiConsole.Write(new Rule("[lightseagreen]🖨️ GENERANDO INFORME HTML DE DOCENTES[/]").RuleStyle(Color.LightSeaGreen));
...
void ImportarDatos(IAcademiaService service)
{
    ...
    if (!AnsiConsole.Confirm($"⚠️  Desea importar los datos desde el fiche o: {AppConfig.AcademiaFile}\nEsta acción puede sobrescribir datos existentes. ¿Desea continuar?"))
    {
        AnsiConsole.MarkupLine("[yellow]👋 Operación cancelada.[/]");
        return;
    }
    ...
    AnsiConsole.Progress()
        ...
        .Start(ctx =>
        {
            var task = ctx.AddTask("[cyan]📥 Importando datos...[/]", true);
            ...
            service.ImportarDatos().Match(
                onSuccess: importados => MostrarExito($"✅ Importados {importados} registros."),
                onFailure: error => MostrarError($"☠️ ERROR AL IMPORTAR: {error.Message}")
            );
        });
}
...
void ExportarDatos(IAcademiaService service)
{
    ...
    AnsiConsole.Progress()
        ...
        .Start(ctx =>
        {
            var task = ctx.AddTask("[cyan]📤 Exportando datos...[/]", true);
            ...
            service.ExportarDatos().Match(
                onSuccess: exportados => MostrarExito($"✅ Exportados {exportados} registros."),
                onFailure: error => MostrarError($"☠️ ERROR AL EXPORTAR: {error.Message}")
            );
        });
}
...
void RealizarBackup(IAcademiaService service)
{
    ...
    AnsiConsole.Status().Start("💾 Creando backup...", ctx =>
    {
        service.RealizarBackup().Match(
            ...
            onFailure: error => MostrarError($"☠️ ERROR AL CREAR BACKUP: {error.Message}")
        );
    });
}
...
void RestaurarBackup(IAcademiaService service)
{
    ...
    AnsiConsole.Status().Start("♻️ Restaurando backup...", ctx =>
    {
        service.RestaurarBackup(archivoSeleccionado).Match(
            onSuccess: restaurados => MostrarExito($"✅ Restauración completada. Total registros: {restaurados}"),
            onFailure: error => MostrarError($"☠️ ERROR AL RESTAURAR: {error.Message}")
        );
    });
}
...
void ImprimirTablaPersonas(IEnumerable<Persona> lista)
{
    ...
    foreach (var p in lista)
    {
        ...
        var tipoColor = p is Estudiante ? "lightcyan" : "lightgreen";
        table.AddRow($"{p.Id}", p.Dni, p.NombreCompleto, $"[{cicloColor}]{(p switch { Estudiante e => e.Ciclo.ToString(), Docente d => d.Ciclo.ToString(), _ => "N/A" })}[/]", $"[{tipoColor}]{tipoEmoji} {tipoTexto}[/]");
    }
...
void ImprimirPanelPersona(Persona p)
{
    ...
    if (p is Estudiante e)
    {
        ...
        var notaColor = e.Calificacion >= 5 ? "green" : "red";
        contenido.AppendLine($"[lightseagreen]📝 NOTA:[/]        [{notaColor}]{e.Calificacion.ToString("F2", AppConfig.Locale)}[/]");
    ...
    if (p.CreatedAt != default)
    {
        ...
        contenido.AppendLine($"[lightseagreen]📅 ALTA (LOC):[/]  {p.CreatedAt.ToLocalTime().ToString("g", AppConfig.Locale)}");
        contenido.AppendLine($"[lightseagreen]🔄 MOD  (LOC):[/]  {p.UpdatedAt.ToLocalTime().ToString("g", AppConfig.Locale)}");
