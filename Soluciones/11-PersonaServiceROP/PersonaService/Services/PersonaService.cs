...
public class PersonaService(IPersonaRepository repository, IValidador<Persona> validador, ICache<int, Persona> cache) : IPersonaService
{
    ...
    public Result<Persona, PersonaError> GetById(int id)
    {
        ...
        return Maybe.From(cache.Get(id))
            .ToResult(PersonaErrors.NotFound(id)) // 1. HAPPY PATH: El dato está en CACHÉ (vía rápida)
    ...
    private Result<Persona, PersonaError> GetFromRepository(int id) =>
        Maybe.From(repository.GetById(id))
            .ToResult(PersonaErrors.NotFound(id)) // 1. HAPPY PATH: La persona se encuentra en la Base de Datos
    ...
    private Result<Persona, PersonaError> CheckExists(int id) =>
        Maybe.From(repository.GetById(id))                       // 1. HAPPY PATH: Se consulta existencia en Repositorio
            .ToResult(PersonaErrors.NotFound(id)); // 2. HAPPY PATH: Si existe, seguimos en vía ÉXITO
    ...
    private Result<Persona, PersonaError> CheckEmailIsUnique(Persona persona) =>
        repository.FindByEmail(persona.Email) is null            // 1. HAPPY PATH: No existe nadie con ese email en la BD
            ? Result.Success<Persona, PersonaError>(persona)      // 2. HAPPY PATH: Retornamos ÉXITO con la persona
            : Result.Failure<Persona, PersonaError>(PersonaErrors.AlreadyExists(persona.Email));
    ...
    private Result<Persona, PersonaError> CheckEmailIsUniqueForUpdate(int id, Persona persona)
    {
        ...
        return (existente is null || existente.Id == id)         // 1. HAPPY PATH: El email es nuevo o pertenece a la misma persona
            ? Result.Success<Persona, PersonaError>(persona)      // 2. HAPPY PATH: Validamos la actualización como ÉXITO
            : Result.Failure<Persona, PersonaError>(PersonaErrors.AlreadyExists(persona.Email));
    }
    ...
    private void LogError(PersonaError error)
    {
        // VÍA DE ERROR: Centralización del logging según el tipo de fallo de dominio
        switch (error) {
            case PersonaError.Validation v: _logger.Error("Fallo Validación: {E}", string.Join(", ", v.Errors)); break;
            case PersonaError.AlreadyExists e: _logger.Error("Fallo Unicidad: {Email} ya existe", e.Email); break;
            case PersonaError.NotFound n: _logger.Warning("Fallo Existencia: Id {Id} no hallado", n.Id); break;
        }
    }
}
